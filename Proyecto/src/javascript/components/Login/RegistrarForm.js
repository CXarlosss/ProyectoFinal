  ...
  let checkInterval = setInterval(() => {
    ...
    if (template) {
      ...
      if (!customElements.get("registrar-form")) {
        ...
        console.warn(
          "⚠️ El elemento <registrar-form> ya está definido. Omitiendo redefinición."
        );
      }
    }
...
export class RegistrarForm extends HTMLElement {
  ...
  async onFormSubmit(e) {
    ...
    const nombre = /** @type {HTMLInputElement} */ (
      this.shadowRoot?.getElementById("nombre-usuario")
    )?.value.trim();
    const email = /** @type {HTMLInputElement} */ (
      this.shadowRoot?.getElementById("email-usuario")
    )?.value.trim();
    const telefono = /** @type {HTMLInputElement} */ (
      this.shadowRoot?.getElementById("telefono-usuario")
    )?.value.trim();
    const direccion = /** @type {HTMLInputElement} */ (
      this.shadowRoot?.getElementById("direccion-usuario")
    )?.value.trim();
    const password = /** @type {HTMLInputElement} */ (
      this.shadowRoot?.getElementById("password-usuario")
    )?.value.trim();
    ...
    try {
      ...
      const usuarioExistente =
        Array.isArray(usuariosAPI) &&
        usuariosAPI.some((user) => user.email === email);
      ...
      const apiResponse = await fetch(
        `${location.protocol}//${location.hostname}${API_PORT}/create/users`,
        {
          method: "POST",
      ...
      if (resultado.acknowledged && resultado.insertedId) {
        ...
        console.log(
          "✅ Guardando usuario completo en localStorage:",
          usuarioCompleto
        );
